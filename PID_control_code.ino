...
float KG,EST_error=50.0,EST_prior,MEA_error= 50,EST_now;
int Roll,Pitch,Yaw;
...
int val3=1099;
int val4=1129;
int readBit0=0,readBit1=0,readBit2=0,readBit3=0;
int Roll_prev = 0,Pitch_prev=0;
float igain_roll = 0,igain_pitch=0;
float K_i = 0.00;
float Stablising_value = 1100;
...
void loop()
{
  readBit0 = digitalRead(bit0);
  readBit1 = digitalRead(bit1);
  readBit2 = digitalRead(bit2);
  readBit3 = digitalRead(bit3);
...
*/
  ...
  */
  ...
  float gyro_z = (accel_t_gyro.value.z_gyro - base_z_gyro)/FS_SEL;
//UNCOMMENT IT
  //Serial.println("X omega ");
 // Serial.print(int(gyro_x+0.072));
  //Serial.print(" Y omega ");
  //Serial.println(int(gyro_y+0.04));
  //Serial.print(" Z omega ");
  //Serial.print(int(gyro_z));
  ...
  KG = EST_error/(EST_error+MEA_error);
    EST_now = EST_prior + KG*((accel_z) - EST_prior);
    EST_error = (1-KG)*EST_prior;
    EST_prior = EST_now;
   //Serial.println(((EST_now*0.0005981445)+0.31),1);

    Roll = int(angle_y+0.9)+3;
    Pitch = int(angle_x+0.471);
    Yaw = int(angle_z);
    
   //UNCOMMENT IT 
 /* Serial.print(" Pitch ");
  Serial.print(int(angle_x+0.45));
  Serial.print(F(" Roll "));
  Serial.print(int(angle_y+0.7)+3);
  Serial.print(F(" Yaw "));
  Serial.println(int(angle_z));*/
 
 

   if(readBit0 == LOW && readBit1== HIGH && readBit2 ==LOW && readBit3==LOW)
{
  if(Stablising_value<1900)
  {
    Stablising_value +=0.25;
   
  }
   Balance(Roll,Pitch,Stablising_value,igain_pitch,igain_roll,gyro_x,gyro_y);
 
  }
  
else if(readBit0==HIGH && readBit1==LOW && readBit2==HIGH && readBit3==LOW)
 {
 // igain_pitch = igain_roll+(K_i*abs(Pitch)); 
  //igain_roll = igain_roll+(K_i*abs(Roll));
  Balance(Roll,Pitch,Stablising_value,igain_pitch,igain_roll,gyro_x,gyro_y);
  
 }
 else if(readBit0==LOW && readBit1==LOW && readBit2==LOW && readBit3 == HIGH)
 { 
    Stablising_value -=0.25;
     Balance(Roll,Pitch,Stablising_value,igain_pitch,igain_roll,gyro_x,gyro_y);
   
}
else if(readBit0 ==HIGH && readBit1 ==LOW && readBit2==LOW && readBit3 ==LOW)
{
  Stablising_value = 0;
   Balance(Roll,Pitch,Stablising_value,igain_pitch,igain_roll,gyro_x,gyro_y);
  }
...
void Balance(int Roll,int Pitch,int Stablising_value,float igain_roll,float igain_pitch,float gyro_x,float gyro_y)
{
  float K_p = 1.0;
  float K_d = 0.3;
  float pgain_roll,dgain_roll,pgain_pitch,dgain_pitch;
  float correction_roll,correction_pitch,higher_val_roll,lower_val_roll,higher_val_pitch,lower_val_pitch;
  
  //Calculating PID gain values
  pgain_roll = K_p*(abs(Roll));
  pgain_pitch = K_p*(abs(Pitch)); 

  correction_roll = (pgain_roll);
  //Serial.println(correction_roll);
  correction_pitch = (pgain_pitch);

  higher_val_roll = Stablising_value + (correction_roll);
  lower_val_roll = Stablising_value - (correction_roll);
  higher_val_pitch = Stablising_value +(correction_pitch);
  lower_val_pitch = Stablising_value - (correction_pitch);

if(gyro_y>=0)
{
 if(Roll<=0)
  {
    Motor1.writeMicroseconds(lower_val_roll-13.5+(K_d*gyro_y));
    Motor3.writeMicroseconds(higher_val_roll-(K_d*gyro_y));
    
    }
  else if(Roll>0)
  {
    Motor3.writeMicroseconds(lower_val_roll-(K_d*gyro_y));
    Motor1.writeMicroseconds(higher_val_roll-13.5+(K_d*gyro_y));
...
else if(gyro_y<0)
{
  if(Roll<=0)
  {
    Motor1.writeMicroseconds(lower_val_roll-13.5-(K_d*(abs(gyro_y))));
    Motor3.writeMicroseconds(higher_val_roll+K_d*(abs(gyro_y)));
    
    }
  else
  {
    Motor3.writeMicroseconds(lower_val_roll+(K_d*(abs(gyro_y))));
    Motor1.writeMicroseconds(higher_val_roll-13.5-(K_d*(abs(gyro_y))));
...
if(gyro_x>=0)
{
    if(Pitch<=0)
    {
      Motor2.writeMicroseconds(lower_val_pitch+(K_d*gyro_x));
      Motor4.writeMicroseconds(higher_val_pitch+29-(K_d*gyro_x));
      }
      else
      {
        Motor2.writeMicroseconds(higher_val_pitch+(K_d*gyro_x));
        Motor4.writeMicroseconds(lower_val_pitch+29-(K_d*gyro_x));
        }
  ...
  else if(gyro_x<0)
  {
    if(Pitch<0)
    {
      Motor2.writeMicroseconds(lower_val_pitch-(K_d*abs(gyro_x)));
      Motor4.writeMicroseconds(higher_val_pitch+29+(K_d*abs(gyro_x)));
      }
      else
      {
        Motor2.writeMicroseconds(higher_val_pitch-(K_d*abs(gyro_x)));
        Motor4.writeMicroseconds(lower_val_pitch+29+(K_d*abs(gyro_x)));
        }
